...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-50 w-full shadow-md">
      <div
        className={`w-full py-1 border-b select-none transition-colors duration-300 ${
          isDarkMode
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-end gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 bg-slate-800 dark:bg-slate-955 p-1 px-2 rounded-md border border-slate-700/60 max-h-7">
              <span className="text-[8.5px] text-slate-400 uppercase tracking-wider font-extrabold">
                {t("roleLabel")}
              </span>
              <select
                value={currentRole}
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2.5 shrink-0">
              ...
              <div className="relative w-[85px] xs:w-[115px] sm:w-[170px] md:w-[220px] shrink-0">
                ...
                {showSuggestions && recentSearches.length > 0 && (
                  <div
                    className={`absolute left-0 right-0 mt-2 p-2 rounded-2xl border shadow-2xl z-50 text-[11px] font-semibold animate-in fade-in slide-in-from-top-1 duration-150 ${
                      isDarkMode
                  ...
                  >
                    <div className="flex items-center justify-between px-2 pb-1.5 mb-1.5 border-b border-solid border-slate-200/50 dark:border-slate-800/60">
                      <span className="text-[9px] font-black uppercase tracking-wider text-orange-500 flex items-center gap-1 whitespace-nowrap">
                        <History className="w-3 h-3 text-orange-500 shrink-0" />{" "}
                        {t("recentSearches")}
                      </span>
                      <button
                  ...
                  <div className="w-7.5 h-7.5 rounded-full overflow-hidden border border-slate-300 dark:border-slate-700 shadow-sm bg-orange-900 flex items-center justify-center shrink-0">
                    {currentRole === "admin" ? (
                      ...
                      <img
                        referrerPolicy="no-referrer"
                        src={
                          showroomProfile?.avatarUrl ||
                          "https://images.unsplash.com/photo-1562575308-9e672757d475?auto=format&fit=crop&q=80&w=64&h=64"
                        }
                        alt="Showroom"
                        className="w-full h-full object-cover"
                      />
